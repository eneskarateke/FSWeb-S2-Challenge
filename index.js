...
/* ÖRNEK GÖREV:
	cumleKur fonksiyonuna aşağıdakileri uygulayın:
		...
		Bu durumda parametre olarak bir şey göndermediğimiz için fonksiyon içindeki degisken değişkenine false değeri atanacaktır. Bu durumda foo(); fonksiyonunu çalıştırdığımız false döndürür.
		)
...
//		Sayfanın en üstünde global olarak tanımlanmış `cumleler` adında bir dizi bulunmaktadır. Bu dizinin içinde en çok 5 en az 1 adet string bulunan diziler bulunmaktadır. 
...
/* 	GÖREV 1:
		cumlelereDonustur fonksiyonuna aşağıdaki yönergeleri uygulayın.
			...
			3. Dizinin içindeki her dizi elemanı 1 cümle oluşturacak şekilde uc uca eklenecek, kelimelerin aralarına 3. parametrede girilen ayraç yerleştirilecek;
				NOT: cumlelereDonustur(cumleler, " ") fonksiyonu çağırıldığında şu dizinin oluşturacağı cümle: ["Annem","ekmek","almak","için","gitti."] => "Annem ekmek almak için 
				gitti." şeklinde olmalıdır. 
	...
	*/
	function cumlelereDonustur(dizi, ayrac = ",") {
		return dizi.map(satir => satir.join(ayrac));
		}
...
// 		let y = dizi[i].join(ayrac)
// 		cumlelerDizisi.push(y)
// 	}
...
/* GÖREV 2:
		paragrafOlustur fonksiyonuna aşağıdakileri uygulayın.
			...
			5. Oluşturulan bu dizinin tek sayılı anahtarlarını(index) [1,3,5,7,9] kullanarak ilk 5 elemanı geriçağırım olarak çağırılan cumleKur dizisinin parametreleri olarak aktarılarak 
			çağırılacak ÖRNEK: callback(dizi[1],dizi[2],dizi[3],dizi[5],dizi[7])
...
Tavşanla kirpi sebzeleri ele geçirmek için bir plan kurdular. Tavşan diziye önden saldıracak, kirpi ise arkalarından dolaşacak. Varsayalım ki arkadaşların planları başarılı oldu. 
...
/* 	GÖREV 4:
		...
		Kullanıcının gönderdiği mesaj stringi içinde emojiler nesnesinin anahtarı(index) bulunuyorsa, bu işareti otomatik olarak anahtara ait değerde tanımlanmış emoji 
		ile değiştirecek bir fonksiyon geliştirmek istiyorlar. ÖRNEK: Mesaj içinde ":)" sembolü bulunursa mesajı alan kişi bu sembolü "🙂" olarak görecek. Burdan yola çıkarak 
 ...
 */
	...
	const emojileriDonustur = (mesaj, smileys) =>
	Object.entries(smileys).reduce((yeniString, [sembol, emoji]) =>
		yeniString.replaceAll(sembol.toUpperCase(), emoji).replaceAll(sembol.toLowerCase(), emoji),
		mesaj
	);
